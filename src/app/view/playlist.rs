use iced::widget::{
    button, column, container, horizontal_space, hover, row, scrollable, stack,
    text_input, vertical_space,
};

use super::*;
use crate::app::{App, playlist::Playlist};
...
impl App {
    fn new_playlist_menu(&self) -> Element {
        container(
            column![
                ...
                horizontal_space().height(5),
                row![
                    text_input("Cover image path", &self.new_playlist_img)
    ...
    fn playlist_list_item_view(playlist: Playlist) -> Element<'static> {
        ...
        .on_press(Message::ViewPlaylist(Some(xxhash_rust::xxh3::xxh3_64(
            playlist.filename.as_bytes(),
        ))))
        .into()
    }
