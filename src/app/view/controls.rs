...
impl App {
    pub(super) fn current_track(&self) -> Element {
        column![self.current_title(), self.current_artist(),]
    ...
    fn current_title(&self) -> Element {
        text!(
            "{}",
            self.playing.as_ref().map_or("", |track| {
                match &track.metadata.title {
                    Some(title) => title,
                    None => track.path.file_name().unwrap().to_str().unwrap(),
                }
            })
        )
        .size(TEXT_SIZE)
    ...
    fn current_artist(&self) -> Element {
        text!(
            "{}",
            self.playing.as_ref().map_or(String::from(""), |track| {
                print_artists(&track.metadata.artists)
            })
        )
        .size(TEXT_SIZE)
        .center()
    ...
    pub(super) fn progress_bar(&self) -> Element {
        let (current_time, total_duration) = match self.track_duration {
            ...
            None => (Duration::from_secs(0), Duration::from_secs(0)),
        };
        row![
            iced::widget::Space::with_width(iced::Length::Fill),
            fill![
                text!("{}", print_duration(&current_time))
            ...
            fill![
                text!("{}", print_duration(&total_duration))
                    .size(SMALL_TEXT_SIZE)
                    .center()
            ]
            .align_x(iced::Alignment::Center)
